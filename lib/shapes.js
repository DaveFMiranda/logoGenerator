class Shape {
  constructor(version, width, height, xmlns, fill) {
  ...
  log(){
    console.log(this.version, this.width, this.height, this.xmlns, this.fill);
  }
}
...
const newShape = new Shape('1.1', '300', '200', 'http://www.w3.org/2000/svg', answers) //.shapeColor)

console.log(newShape);

class Rectangle extends Shape {
  constructor(newShape, {x, y, rectWidth, rectHeight}){
  super(newShape.version, newShape.width, newShape.height, newShape.xmlns, newShape.fill);
  this.x = x;
  this.y = y;
...
class Triangle extends Shape {
  constructor(newShape, {left, apex, right}){
  super(newShape.version, newShape.width, newShape.height, newShape.xmlns, newShape.fill);
  this.left = left;
  this.apex = apex;
  this.right = right;
...
function renderRectangle(width, height, version, xmlns, fill){
...
rectHeight: 100
  ...
  const rectangleHTML = '<svg width="'+width+'" height="'+height+'" version="'+version+'" xmlns="'+xmlns+'">';
  console.log(rectangleHTML);
}

function renderCircle(width, height, version, xmlns, fill){
...
r: 150
  ...
  const circleHTML = '<svg width="'+width+'" height="'+height+'" version="'+version+'" xmlns="'+xmlns+'">';
  console.log(circleHTML);
}

function renderTriangle(width, height, version, xmlns, fill){
...
right: '300 200'
  ...
  const triangleHTML = '<svg width="'+width+'" height="'+height+'" version="'+version+'" xmlns="'+xmlns+'">'+'<polygon points="'+newTriangle.left+newTriangle.apex+newTriangle.right+'" fill="'+fill+'"/></svg>'
  console.log(triangleHTML);
}

renderRectangle(newShape.width, newShape.height, newShape.version, newShape.xmlns, newShape.fill)
renderTriangle(newShape.width, newShape.height, newShape.version, newShape.xmlns, newShape.fill)
renderCircle(newShape.width, newShape.height, newShape.version, newShape.xmlns, newShape.fill)

// NOW FINISH THE STRINGS FOR RECTANGLE AND CIRCLE
